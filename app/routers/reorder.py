...
def create_suggested_reorders(
    ...
    # Non-dry runs still require explicit user approval and quantity confirmation.
    db.commit()
    return SuggestedReorderResult(proposal_id=proposal.id, created=created, skipped=skipped)
...
def approve_reorder_proposal(
    ...
    quantities_by_item_id = {item.item_id: item.quantity_added for item in payload.item_quantities}

    for item in items:
        approved_qty = quantities_by_item_id.get(item.id)
        if approved_qty is None:
            blocked.append(
                ReorderApprovalBlockedItem(
                    item_id=item.id,
                    product_id=item.product_id,
                    reason="missing user-approved quantity for proposal item",
                )
            )
            continue
        ...
        item.quantity_added = approved_qty
        item.quantity_after = item.quantity_before + approved_qty
        product.quantity_on_hand += approved_qty
        db.add(
            InventoryLot(
                product_id=product.id,
                quantity_remaining=approved_qty,
            )
        )
        db.add(
            StockMovement(
